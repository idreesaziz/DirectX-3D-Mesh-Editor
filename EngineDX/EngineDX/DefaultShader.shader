...
// 2 -> OnEvent, is only triggered on certian events. it is not hot / rarely updated
...
PSInput VShader( VSInput input )
{
	...
	PSInput output = (PSInput)0;
		...
		output.Norm 		= mul(float4(input.Norm, 1), View).xyz;
		output.Norm 		= mul(float4(input.Norm, 1), Projection).xyz;	return output;
}
...
{
	float4 outColor = input.HemiAmbient;

	// calculating the directional lighting
	outColor += saturate(dot((float3)vLightDirection, input.Norm)) * vLightColor * 4;
	outColor.a = 1;
